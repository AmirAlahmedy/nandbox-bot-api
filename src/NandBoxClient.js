...
class NandBoxClient {
  ...
  connect(token, callback) {
    ...
    setWSCallbacks(this.session, token);
  }
}
...
class InternalWebSocket {
  ...
  reconnectWebSocketClient() {
    ...
    setWSCallbacks(this, this.token)
...
function setWSCallbacks(internalWS, token) {
  internalWS.ws.onopen = function () {
    internalWS.closingCounter = 0
    console.log("INTERNAL: ONCONNECT");
    Logger.logger.info("INTERNAL: ONCONNECT");
    setApiMethods(internalWS, api);
    internalWS.sendAuthToken(token);
    internalWS.pingpong();
  };
  internalWS.ws.onclose = async function (status) {
    ...
    // TODO: pingpong here
    clearInterval(internalWS.pingpong);

    internalWS.callback.onClose();

    if (
      (status.code == 1000 || status.code == 1006 || status.code == 1001 || status.code == 1005) &&
      internalWS.closingCounter < internalWS.NO_OF_RETRIES_IF_CONN_CLOSED
    ) {
      try {
        ...
        internalWS.closingCounter++;
        console.log("Conenction Closing counter is: " + internalWS.closingCounter);
        Logger.logger.info("Conenction Closing counter is: " + internalWS.closingCounter);
      } catch (e1) {
        console.log(e1);
        Logger.logger.error(e1);
      }

      internalWS.stopWebSocketClient();
      try {
        internalWS.reconnectWebSocketClient();
      } catch (e) {
        console.log(e);
        Logger.logger.error(e);
      }
  ...
  internalWS.ws.onerror = function (error) {
    console.log("ONERROR: ", error);
    Logger.logger.error("ONERROR: " + error);
  };

  internalWS.ws.onmessage = function (msg) {
    let user;
    internalWS.lastMessage = new Date().getUTCMilliseconds();
    ...
    if (method) {
      ...
      switch (method) {
        case "TOKEN_AUTH_OK":
          ...
          internalWS.callback.onConnect(api, obj);

          return;
        case "message":
          let incomingMessage = new IncomingMessage(obj);
          internalWS.callback.onReceive(incomingMessage);
          return;
        case "chatMenuCallback":
          let chatMenuCallback = new ChatMenuCallback(obj);
          internalWS.callback.onChatMenuCallBack(chatMenuCallback);
          return;
        case "inlineMessageCallback":
          let inlineMsgCallback = new InlineMessageCallback(obj);
          internalWS.callback.onInlineMessageCallback(inlineMsgCallback);
          return;
        case "inlineSearch":
          let inlineSearch = new InlineSearch(obj);
          internalWS.callback.onInlineSearh(inlineSearch);
          return;
        case "messageAck":
          let msgAck = new MessageAck(obj);
          internalWS.callback.onMessagAckCallback(msgAck);
          return;
        case "userJoinedBot":
          user = new User(obj.user);
          internalWS.callback.onUserJoinedBot(user);
          return;
        case "chatMember":
          let chatMember = new ChatMember(obj);
          internalWS.callback.onChatMember(chatMember);
          return;
        case "myProfile":
          user = new User(obj.user);
          internalWS.callback.onMyProfile(user);
          return;
        case "userDetails":
          user = new User(obj.user);
          internalWS.callback.onUserDetails(user);
          return;
        case "chatDetails":
          let chat = new Chat(obj.chat);
          internalWS.callback.onChatDetails(chat);
          return;
        case "chatAdministrators":
          let chatAdministrators = new ChatAdministrators(obj);
          internalWS.callback.onChatAdministrators(chatAdministrators);
          return;
        case "userStartedBot":
          user = new User(obj.user);
          internalWS.callback.userStartedBot(user);
          return;
        case "userStoppedBot":
          user = new User(obj.user);
          internalWS.callback.userStoppedBot(user);
          return;
        case "userLeftBot":
          user = new User(obj.user);
          internalWS.callback.userLeftBot(user);
          return;
        case "userLeftBot":
          let permenantURL = new PermanentUrl(obj);
          internalWS.callback.permanentUrl(permenantURL);
          return;
        case "blacklist":
          let blackList = new BlackList(obj);
          internalWS.callback.onBlackList(blackList);
          return;
        case "whitelist":
          let whiteList = new WhiteList(obj);
          internalWS.callback.onWhiteList(whiteList);
          return;
        default:
          internalWS.callback.onReceiveObj(obj);
          return;
      }
    } else {
      let error = obj.error;
